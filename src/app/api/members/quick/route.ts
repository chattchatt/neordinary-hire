...
// In-memory duplicate-click guard: ip+email -> timestamp of last successful submission
...
export async function POST(req: NextRequest) {
  ...
  const discordHandle = sanitize(
    body.discordHandle || body.discordUsername,
  ).slice(0, 80);
  ...
  const employmentTypesRaw = Array.isArray(body.employmentTypes)
    ? body.employmentTypes
    : [];
  const employmentTypes = employmentTypesRaw
    .map((e) => sanitize(String(e)))
    .filter(Boolean);
  ...
  if (employmentTypes.length === 0)
    return NextResponse.json(
      { error: "희망 근무형태를 하나 이상 선택해주세요." },
      { status: 400 },
    );

  // Duplicate-click guard: do not block different people behind the same network/IP.
  ...
  if (lastSubmission && Date.now() - lastSubmission < RATE_LIMIT_WINDOW_MS) {
    ...
    return NextResponse.json(
      {
        error: `방금 접수된 이메일입니다. (${remainingSecs}초 후 다시 확인 가능)`,
      },
      { status: 429 },
    );
  }
  ...
  try {
    ...
    if (isMissingColumnError(error)) {
      return NextResponse.json(
        {
          error:
            "서비스 준비 중입니다. Discord 연동 DB 마이그레이션 적용 후 등록이 가능합니다.",
  ...
  // Record duplicate-click guard timestamp after successful creation
  rateLimitStore.set(rateLimitKey, Date.now());

  return NextResponse.json(
    { success: true, memberId: member.id, hireLinkCode: member.hireLinkCode },
    { status: 201 },
  );
}
